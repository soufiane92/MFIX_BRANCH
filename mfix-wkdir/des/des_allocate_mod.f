...
MODULE DES_ALLOCATE
  ...
  PUBLIC:: DES_ALLOCATE_ARRAYS, ADD_PAIR, ADD_PAIR_ME, PARTICLE_GROW, ALLOCATE_DEM_MI
...
! ====================================================================== !
 ...
 ! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ !
      Allocate(  DES_POS_OLD_ME (MAX_PIP,DIMN) )
...
! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ !
      ...
      Allocate(  NEIGHBORS_OLD_ME (MAX_PIP) )
      Allocate(  PFT_NEIGHBOR_ME (3,MAX_PIP) )
      Allocate(  PFT_NEIGHBOR_OLD_ME (3,MAX_PIP) )
! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ !

! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ !
      Allocate(  PN (MAX_PIP) )
      Allocate(  VM (MAX_PIP) )
      Allocate(  DPN (MAX_PIP) )
      Allocate(  ACTIVE (MAX_PIP) )
! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ !
...
! ==================================================================== !
      ...
      DOUBLE PRECISION FUNCTION add_pair_me(ii,jj)
      USE discretelement
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: ii,jj

      CALL NEIGHBOR_GROW_ME(NEIGHBOR_INDEX_ME(ii))

      NEIGHBORS_ME(NEIGHBOR_INDEX_ME(ii)) = jj
      NEIGHBOR_INDEX_ME(ii) = NEIGHBOR_INDEX_ME(ii) + 1
      add_pair_me = NEIGHBOR_INDEX_ME(ii)

      RETURN
      END FUNCTION add_pair_me
...
! ==================================================================== !
      ...
      SUBROUTINE NEIGHBOR_GROW_ME(new_neigh_max)
        USE discretelement
        USE geometry
        IMPLICIT NONE
        ...
        allocate(neigh_tmp(new_size))
        neigh_tmp(1:lSIZE1) = neighbors_me(1:lSIZE1)
        neigh_tmp(lSIZE1+1:) = 0
        call move_alloc(neigh_tmp,neighbors_me)

        allocate(neigh_tmp(new_size))
        neigh_tmp(1:lSIZE1) = neighbors_old_me(1:lSIZE1)
        neigh_tmp(lSIZE1+1:) = 0
        call move_alloc(neigh_tmp,neighbors_old_me)

        allocate(pf_tmp(3,new_size))
        pf_tmp(:,1:lSIZE1) = pft_neighbor_me(:,1:lSIZE1)
        pf_tmp(:,lSIZE1+1:) = 0
        call move_alloc(pf_tmp,pft_neighbor_me)

        allocate(pf_tmp(3,new_size))
        pf_tmp(:,1:lSIZE1) = pft_neighbor_old_me(:,1:lSIZE1)
        pf_tmp(:,lSIZE1+1:) = 0
        call move_alloc(pf_tmp,pft_neighbor_old_me)


      END SUBROUTINE NEIGHBOR_GROW_ME
