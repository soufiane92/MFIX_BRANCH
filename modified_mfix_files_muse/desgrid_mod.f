...
! ==================================================================== !    
    ...
    subroutine desgrid_neigh_build_me ()
...
!-----------------------------------------------
      ...
      use compar
      use constant
      use des_allocate, only: add_pair_me
      use desmpi_wrapper
      use derived_types, only: dg_pic
      use discretelement
      use funits
...
!$  if (.false.) then
            if (NEIGHBOR_INDEX_ME(lcurpar) .eq. 0) then
                if (lcurpar .eq. 1) then
                    NEIGHBOR_INDEX_ME(lcurpar) = 1
                else
                    NEIGHBOR_INDEX_ME(lcurpar) = NEIGHBOR_INDEX_ME(lcurpar-1)
                endif
            endif
!$  endif
         ...
         lcurpar_pos(:) = des_pos_new_me(lcurpar,:)
!   The desgrid size should not be less than 2*dia*rlm_factor
...
!$                NEIGHBOR_INDEX(dd) = NEIGHBOR_INDEX(dd-1)
!$            enddo
!$            cc = add_pair_me(lcurpar, PAIRS_SMP(2,MM))
!$        enddo
!$omp end ordered
...
!$omp end parallel
    ...
    end subroutine desgrid_neigh_build_me
